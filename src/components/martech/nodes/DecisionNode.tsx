...
export function DecisionNode({ node, selected }: { node: DecisionNodeModel; selected: boolean }) {
  ...
  return (
    <div
      className={`flex h-full w-full items-center gap-2 rounded-control border px-2.5 ${evidenceClasses(node.evidence)}`}
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-[12px] font-semibold leading-tight" title={node.label}>
          {node.label}
        </span>
        <span className="block truncate text-[9.5px] uppercase tracking-[.04em] text-t3">
